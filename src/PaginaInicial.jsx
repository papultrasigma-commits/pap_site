...
import AuthGate from "./ControloAutenticacao.jsx";
import { supabase } from "./clienteSupabase";

// 🚀 A MAGIA ACONTECE AQUI: Guardamos o link original antes de o React Router alterar
const LINK_ORIGINAL = window.location.href;
...
export default function HomePage() {
  ...
  useEffect(() => {
    // Usamos o LINK_ORIGINAL em vez do window.location.href para garantir que apanhamos os tokens
    const isRecovery = 
      LINK_ORIGINAL.includes("type=recovery") || 
      LINK_ORIGINAL.includes("recovery") || 
      LINK_ORIGINAL.includes("update-password") || 
      LINK_ORIGINAL.includes("access_token=") || 
      LINK_ORIGINAL.includes("code=");
    ...
    supabase.auth.getSession()
      .then(({ data }) => {
        if (data.session || sessionStorage.getItem("force_login_form") === "true" || isRecovery) {
          setShowAuth(true);
        }
      })
      .catch(console.error)
...
@media (max-width: 1440px) {
  .hero { min-height: 60vh; padding: 6rem 5% 3rem; }
  .hero h1 { font-size: 3rem; margin-bottom: 1rem; }
  .hero h1 .subtitle { font-size: 2.2rem; }
  .hero p { font-size: 1rem; margin-bottom: 2rem; max-width: 700px; }
  .hero-buttons { margin-bottom: 3rem; }
  .stat-card { padding: 1.2rem; }
  .stat-number { font-size: 2rem; }
  .section-title { font-size: 2rem; }
  .step-card, .feature-card, .tournament-type-card { padding: 1.5rem; }
  .step-card h3, .feature-card h3, .tournament-type-card h3 { font-size: 1.2rem; }
  .honor-way-card, .honor-level-card { padding: 1rem; gap: 1rem; }
  .tournament-item { padding: 1.5rem; gap: 1.5rem; }
  .how-it-works, .features, .honor-system, .tournaments { padding: 4rem 5%; }
}
      ...
      <section id="como-funciona" className="how-it-works">
        ...
        <div className="steps-grid">
          ...
          <div className="step-card">
            ...
            <p>Agenda treinos, develops estratégias e participa em torneios.</p>
          </div>
        </div>
      </section>
